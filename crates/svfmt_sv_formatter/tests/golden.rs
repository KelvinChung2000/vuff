...
//!    options exposed by `svfmt_config::FormatOptions`.
...
use svfmt_config::{BeginStyle, FormatOptions, IndentStyle, PortListStyle, TrailingComma};
use svfmt_sv_formatter::format_source;
...
fn parse_golden(src: &str) -> Result<Golden, String> {
    ...
    Ok(Golden { options, input, expected, xfail_reason })
}
...
fn run_one(path: &Path) -> Result<Outcome, String> {
    ...
    let Golden { options, input, expected, xfail_reason } =
        parse_golden(&src).map_err(|e| format!("parse {}: {e}", path.display()))?;
    let got = format_source(&input, &options)
        .map_err(|e| format!("format {}: {e}", path.display()))?;
...
mod parser_tests {
    ...
    fn comma_separated_config() {
        let src = "// config: indent_width=4, line_width=80\nmodule m;\n// expected -----\nmodule m;\n";
        let g = parse_golden(src).unwrap();
        assert_eq!(g.options.indent_width, 4);
