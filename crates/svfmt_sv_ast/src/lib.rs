...
pub fn parse(source: &str, path: &Path) -> Result<Parsed, ParseError> {
    ...
    let (pp, defines_out) =
        preprocess_str(source, path, &defines, &includes, false, false, 0, 0)
            .map_err(|e| ParseError::Preprocess(format!("{e:?}")))?;
    let text = pp.text().to_owned();
    let (tree, _) = parse_sv_pp(pp, defines_out, false)
...
pub enum RoundTripError {
    ...
    #[error("reconstructed ({reconstructed} bytes) differs from preprocessed source ({original} bytes)")]
    LengthMismatch {
        reconstructed: usize,
        original: usize,
    },
