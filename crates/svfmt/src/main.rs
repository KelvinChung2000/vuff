//! `svfmt` — the unified SystemVerilog formatter + (eventually) linter CLI.
...
use svfmt_config::{load_config, ConfigSource, FormatOptions, ResolvedConfig};
...
#[command(
    name = "svfmt",
    version,
    about = "SystemVerilog formatter (ruff-style)"
)]
struct Cli {
    #[command(subcommand)]
    command: Cmd,
...
fn run_stdin(opts: &FormatOptions, args: &FormatArgs) -> Result<u8> {
    ...
    std::io::stdin()
        ...
        .context("read stdin")?;
    let out = svfmt_sv_formatter::format_source(&buf, opts).context("format stdin")?;
    if args.check {
...
fn process_file(path: &Path, opts: &FormatOptions, args: &FormatArgs) -> Result<bool> {
    ...
    let out = svfmt_sv_formatter::format_source(&src, opts)
